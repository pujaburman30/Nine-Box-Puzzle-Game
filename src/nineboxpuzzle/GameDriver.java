...
public class GameDriver {
    ...
    private ScoreBoard statistics;

    GameDriver(PlayArea playArea, ScoreBoard statistics) {
    ...
    public void makeMove(int positionX, int positionY) {
        if (canMakeMove(positionX, positionY)) {
            ...
            emptyBox.setBackground(Color.WHITE);//Done
            box.setEnabled(false);
            box.setText("");
            box.setBackground(Color.LIGHT_GRAY);//Done
            emptyBox = box;
    ...
    private Box[][] getRandomBoxes() {
        ...
        init = 1;
        emptyBox.setBackground(Color.LIGHT_GRAY); //Done
        return boxes;
    }

    void setPlayerData(String name) {
        PlayerDetails playerData = new PlayerDetails(name, movesCount);
